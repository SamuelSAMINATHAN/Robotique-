...
def track_robot(target: RobotInfo, chassis):
...
def search_for_robot(chassis, last_dx):
...
if __name__ == '__main__':
    ...
    ep_vision.sub_detect_info(name="robot", callback=on_detect_robot)
    ...
    try:
        while True:
            ...
            if robots:
                target = robots[0]
                dx, dy = track_robot(target, ep_chassis)
                ...
                tracking_started = False
                search_for_robot(ep_chassis, last_dx)

            cv2.imshow("Robot Tracking Final", img)
            if cv2.waitKey(1) == 27:
                break

    finally:
        with open("robot_tracking_errors.json", "w") as f:
